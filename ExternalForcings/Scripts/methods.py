...
for ifc,ncname in enumerate(files):
    ...
    axs[0].plot(H,C,c=colors[ifc],label=conf[ifc])
    axs[1].plot(taux/5000*365,H,c=colors[ifc],linestyle='dashed',label=r'$H_{S}$')
    axs[1].plot(taux/5000*365,C,c=colors[ifc],linestyle='dotted',label=r'$C_{JS}$')
...
axs[1].set_xlabel(r'embedding delay $[days]$')
axs[1].legend()
fig.tight_layout()
