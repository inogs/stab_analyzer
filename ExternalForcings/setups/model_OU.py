...
k=0
l=[]
trials = 100  #stochastich ensemble
...
y = np.zeros((trials,ntrials,len(t_eval),54))
...
#     tau = tau_days*60*60/2/3.14 #Time constant
      ...
      YearlyCycle =  np.zeros((n,trials))
      for i in range(trials):
          YearlyCycle[:,i] = YC
      T = np.zeros((n,trials))
      Ft = np.zeros((n,trials))
      F = np.zeros(trials)
      for i in range(n):
      ...
      # Final series
      T = MeanTemp + YearlyCycle + Ft  #red noise over constant temperature
      ...
      model.dependencies['temperature'].value = T[0,0]#15.
...
#     model.dependencies['surface_downwelling_shortwave_flux'].value = L[0]#50.
      ...
      for jj in range(trials): 
        ...
        if r == rank :
          ...
          sqrtdt = np.sqrt(dt)
          y[jj,s,0,:]=model.state[:]
          
          #print(model.state) 
          for i in range(len(t_eval)):
              model.dependencies['temperature'].value = T[i,jj]
    #         model.dependencies['surface_downwelling_shortwave_flux'].value = L[i]
              dy = model.getRates()
              for j in range(len(model.state)):
                  if i!=0:
                      y[jj,s,i,j]=y[jj,s,i-1,j]+dy[j]*dt
              model.state[:]=y[jj,s,i,:]
          #if s!=ntrials-1:
          #    del model
if rank == 0:
    for jj in range(trials):
        ...
        if r !=0 :
          ...
          y[int(dic['idx'])] = dic['data']
else:
    for jj in range(trials):
        r = jj % nranks
        dic = {'idx':jj, 'data':y[jj]}
        if r == rank :
