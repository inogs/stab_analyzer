...
for i,name in enumerate(target):
    if name.find(key) !=-1 or name.find(keyOc) !=-1 or name.find(keyOh) !=-1 or name.find(keyR3c) !=-1:
...
for inc,ncname in enumerate(filenames_cycle) :
    ...
    if i == rank :
        ...
        if -sh <= np.log(9):
            ...
            count_s_c =+1
          #  txt_file.write(str(-sh) + '\n')
        #else :
        #    count_s_c =+1
        #    for pv in p_cycles:
        #       if pv/np.sum(p_cycles) > 1 :
        if inc == 98:
            tot = []
            for ip,pv in enumerate(p_cycles):
                if pv ==0:
                    ...
                    tot.append(pv/np.sum(p_cycles)*np.log(pv/np.sum(p_cycles)))
            txt_file.write(str(np.sum(tot)) + '\n')
            txt_file.write(str(-sh)+'\n')
