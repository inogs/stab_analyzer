...
class Job(shared.Job2):
    ...
    def evaluate2(self, values, i, extra_outputs=None, return_model_values=False, show_output=False):
        assert self.started
        print(str(i))
        ...
        returncode = run_program(self.exe, self.scenariodir, i, use_shell=self.use_shell, show_output=show_output, verbose=self.verbose)
    ...
    def prepareEnsembleDirectories(self, ensemble, root, format='%04i', symlink=False):
        ...
        for i, simulationdir in enumerate(dir_paths):
            ...
            fid=open(simulationdir+'/ensamble_counter.txt','w')
            print(simulationdir+'/ensamble_counter.txt')
            fid.write(str(i))
            fid.close()
        return dir_paths
    ...
    def runEnsemble(self, directories, ncpus=None, ppservers=(), socket_timeout=600, secret=None, show_output=False):
        ...
        if self.verbose:
            print('Total number of cpus: pippo %i' % nworkers)
        if nworkers == 0:
            raise Exception('No cpus available; exiting.')
        jobs = []
        print('pippo')
        print(self.scenariodir)
        print(directories[0])
        for idx,rundir in enumerate(directories):
...
def run_program(exe, rundir,i, use_shell=False, show_output=True, verbose=True):
    if verbose:
        time_start = time.time()
        print('Starting model run... ' + rundir)
    fid=open(rundir+'/ensamble_counter.txt','w')
    print(rundir+'/ensamble_counter.txt')
    fid.write(str(i))
    fid.close()
